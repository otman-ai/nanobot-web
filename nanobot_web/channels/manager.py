...
class ChannelManager:
    ...
    def _validate_allow_from(self) -> None:
        ...
        for name, ch in self.channels.items():
            if getattr(ch.config, "allow_from", None) == []:
                logger.warning(
                    '"{}" has empty allowFrom — skipping channel. '
                    "Add user IDs in the channels config to enable it.",
                    name,
                )
                skip.append(name)
        for name in skip:
    ...
    async def _dispatch_outbound(self) -> None:
        ...
        while True:
            try:
                ...
                channel = self.channels.get(msg.channel.lower())
                if channel:
                    try:
    ...
    async def reload_config(self, new_config: Config) -> None:
        """Hot-reload channels when config changes."""
        from nanobot_web.channels.registry import discover_channel_names, load_channel_class

        old_names = set(self.channels.keys())
        self.config = new_config

        groq_key = new_config.providers.groq.api_key

        # Discover what should be enabled now
        desired: dict[str, Any] = {}
        for modname in discover_channel_names():
            section = getattr(new_config.channels, modname, None)
            if section and getattr(section, "enabled", False):
                desired[modname] = section

        # Stop removed channels
        for name in old_names - set(desired.keys()):
            ch = self.channels.pop(name, None)
            if ch:
                try:
                    await ch.stop()
                    logger.info("Stopped removed channel: {}", name)
                except Exception as e:
                    logger.error("Error stopping {}: {}", name, e)

        # Start new channels
        for name, section in desired.items():
            if name not in self.channels:
                try:
                    ...
                    logger.info("Starting new channel: {}", cls.display_name)
                    asyncio.create_task(self._start_channel(name, channel))
                except ImportError as e:
                    logger.warning("{} channel not available: {}", name, e)

        # Ensure dispatcher is running
        if self.channels and (not self._dispatch_task or self._dispatch_task.done()):
            self._dispatch_task = asyncio.create_task(self._dispatch_outbound())
