...
import nanobot_web.channels.matrix as matrix_module
from nanobot_web.bus.events import OutboundMessage
from nanobot_web.bus.queue import MessageBus
from nanobot_web.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from nanobot_web.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("nanobot_web.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "nanobot_web.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("nanobot_web.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "nanobot_web.channels.matrix.asyncio.create_task", _fake_create_task
    )
